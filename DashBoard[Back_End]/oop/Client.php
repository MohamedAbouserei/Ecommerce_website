...
class Client extends Person {
   ...
   function setID($iD) {
    ...
    public function addToCart ($client_id , $service_id , $date , $shoppingCart)
    {
        $this->connect();
        return $shoppingCart->AddtoCart($client_id,$service_id,$date , $this->db_connection);

    }

    public function placeOrder (Order $o , $service_id)
    {
        $this->connect();
        return $o->placeOrder($this->getID(), $service_id, $this->db_connection);
    }
    ...
    public function showServiceEstimates($service)
    {
        $this->connect();
        return $service->showServiceEstimates($this->db_connection);
    }
    // update coins
    ...
    /********************* Payment methods ********************************/
    // View Coins
    public function viewcoins($id)
    {
       $user = new Client();
       $user->connect();
       $sql="SELECT  `coins` FROM `customer` WHERE `c_id`=$id";
        return $user->db_connection->select($sql,null);
    }
