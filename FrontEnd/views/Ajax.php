...
if($_SERVER['REQUEST_METHOD']=='POST')
{
    ...
    if($_POST["action"]=="updateCoins" && isset($_POST["c_id"]) && isset($_POST["s_price"]) && isset($_POST["s_id"]))
    {
      ...
      $check =  $sc->placeServiceToOrder($o , $client , $_POST["s_id"]);
    ...
    if($_POST["action"]=="addEstimate" && isset($_POST["o_id"]) && isset($_POST["s_id"]))
    {
      $client = unserialize($_SESSION['Client']);
      $e = new Estimate();
      echo $e->setOrderEstimate($_POST["o_id"],$client->getID(),$_POST["s_id"],$_POST["comment"],$_POST["stars"],$client->db_connection);
    }
    ...
    if($_POST['action']=="addToshoppingCart" && isset($_POST['client_id']) && isset($_POST['service_id']) && isset($_POST['date']))
    {
        ...
        echo  $client->addToCart($_POST['client_id'] , $_POST['service_id'] ,$_POST['date'] , $shoppingCart);
    }
    // ----------------------Place order  ----------------
    if($_POST['action']=="placeOrder" && isset($_POST['service_id']))
    {
        if(isset($_SESSION['Client']))
        {
            ...
            $check = $client->updateCoinse();
            $order = new Order();
            $check = $client->placeOrder($order ,$_POST['service_id']);
            $client->db_connection->disconnect();
            $_SESSION['Client'] = serialize($client);
            echo $check;
        }
                ...
                echo '<!--start service -->
                    <div class="item showService col-xs-4">
                        <div class="thumbnail item-box">
                            ...
                            <div class="caption">
                                <h4 class="text-center" style="font-weight:bolder;">Item 1</h4>
